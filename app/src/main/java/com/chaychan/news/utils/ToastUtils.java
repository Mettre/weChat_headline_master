...
public class ToastUtils {
    ...
    private static void showToast(@StringRes int resId, int duration) {
        showToast(BaseApp.getContext().getResources().getText(resId).toString(), duration);
    }
    ...
    private static void showToast(@StringRes int resId, int duration, Object... args) {
        showToast(String.format(BaseApp.getContext().getResources().getString(resId), args), duration);
    }
     ...
     * @param duration 显示时长
     */
    public static void showCenterToast(CharSequence text, int duration) {
        if (isJumpWhenMore) cancelToast();
        if (sToast == null) {
            sToast = Toast.makeText(BaseApp.getContext(), text, duration);
        } else {
